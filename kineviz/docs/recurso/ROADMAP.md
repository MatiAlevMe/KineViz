    ...
    2.1 [En Progreso] Lógica Central para "Deshacer Eliminación".
        ...
        - Mecanismo:
            - Al confirmar una eliminación, *antes* de la eliminación real:
                1. [En Progreso] Copiar los elementos a eliminar a una caché temporal "undo" (ej. `kineviz/backups/.undo_cache/`). (Implementado para Estudios en `StudyService`; Implementado para Archivos en `FileService`; Implementado para Resultados de Análisis en `AnalysisService`)
                2. [En Progreso] Copiar el archivo `kineviz.db` actual a esta caché "undo". (Implementado vía `UndoManager.prepare_undo_cache()`)
            - [En Progreso] Proceder con la eliminación de los elementos del sistema vivo y actualizar `kineviz.db`. (Integrado en `StudyService` para Estudios; Integrado en `FileService` para Archivos; Integrado en `AnalysisService` para Resultados de Análisis)
