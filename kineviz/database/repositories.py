...
class StudyRepository:
    ...
    def delete_all_studies(self):
        """
        Elimina TODOS los estudios de la base de datos y sus carpetas asociadas.
        ¡Esta acción es irreversible!
        """
        study_names_to_delete = []
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                # Primero, obtener todos los nombres de estudio para eliminar sus carpetas
                cursor.execute('SELECT nombre_estudio FROM estudios')
                study_names_to_delete = [row[0] for row in cursor.fetchall()]

                # Eliminar todos los registros de la tabla estudios
                cursor.execute('DELETE FROM estudios')
                conn.commit()
                logger.info("Todos los registros de estudios han sido eliminados de la base de datos.")
        except sqlite3.Error as e:
            logger.error(f"Error al eliminar todos los estudios de la base de datos: {e}", exc_info=True)
            # No relanzar inmediatamente, intentar eliminar carpetas de todos modos si se obtuvieron nombres.
            # O sí relanzar si se considera crítico no poder acceder a la DB. Por ahora, continuamos.

        # Eliminar todas las carpetas de estudio
        if not study_names_to_delete and self.studies_base_dir.exists():
            # Si no se pudieron obtener nombres de la DB pero la carpeta base existe,
            # intentamos listar las carpetas directamente.
            # Esto es un fallback y puede ser peligroso si studies_base_dir no es lo esperado.
            logger.warning("No se obtuvieron nombres de estudio de la DB. Intentando listar carpetas en studies_base_dir para eliminación.")
            try:
                study_names_to_delete = [d.name for d in self.studies_base_dir.iterdir() if d.is_dir()]
            except Exception as e_list_dir:
                logger.error(f"Error listando directorios en {self.studies_base_dir} para eliminación total: {e_list_dir}")
                # Si falla aquí, no podemos continuar con la eliminación de carpetas.
                # Si la DB se limpió, esto deja un estado inconsistente.
                raise IOError(f"Error crítico: DB limpiada pero no se pudieron listar carpetas de estudio para eliminar. {e_list_dir}")


        deleted_folder_count = 0
        for study_name in study_names_to_delete:
            study_dir_path = self.studies_base_dir / study_name
            if study_dir_path.exists() and study_dir_path.is_dir():
                try:
                    shutil.rmtree(study_dir_path)
                    logger.info(f"Directorio de estudio eliminado: {study_dir_path}")
                    deleted_folder_count += 1
                except OSError as e:
                    logger.error(f"Error al eliminar el directorio del estudio {study_dir_path}: {e}", exc_info=True)
                    # Continuar con otros si es posible
            else:
                logger.warning(f"Directorio del estudio '{study_name}' no encontrado en {study_dir_path}, no se eliminó.")
        
        logger.info(f"Eliminación de todas las carpetas de estudio completada. {deleted_folder_count} carpetas eliminadas.")
        # Considerar si se debe crear de nuevo la carpeta 'estudios' base si se eliminó por error.
        # self.studies_base_dir.mkdir(exist_ok=True) # No, rmtree no elimina la base si es el target.
