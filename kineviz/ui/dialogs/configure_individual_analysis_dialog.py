...
class ConfigureIndividualAnalysisDialog(tk.Toplevel):
    ...
    def __init__(self, parent, analysis_service: AnalysisService, study_id: int):
        ...
        self.paired_var = tk.BooleanVar(value=False)   # Asumir independiente por defecto
        ...
        self.selected_group_vars = [] # Lista de StringVars para grupos seleccionados
        self.group_selector_frames = [] # Lista de frames para cada fila de grupo
    ...
    def create_widgets(self):
        ...
        self.freq_combo = ttk.Combobox(param_frame, textvariable=self.frequency_var, values=self.available_frequencies, state="readonly")
        self.freq_combo.grid(row=0, column=1, sticky="ew", padx=5, pady=5)
        self.freq_combo.bind("<<ComboboxSelected>>", self.update_available_groups) # Actualizar grupos al cambiar frecuencia

        ttk.Label(param_frame, text="Cálculo:").grid(row=1, column=0, sticky="w", padx=5, pady=5)
        self.calc_combo = ttk.Combobox(param_frame, textvariable=self.calculation_var, values=self.available_calculations, state="readonly")
        self.calc_combo.grid(row=1, column=1, sticky="ew", padx=5, pady=5)
        self.calc_combo.bind("<<ComboboxSelected>>", self.update_available_columns) # Actualizar columnas al cambiar cálculo

        # --- Selección de Grupos ---
        group_frame = ttk.LabelFrame(main_frame, text="Grupos a Comparar (mínimo 2)")
        ...
        add_group_button = ttk.Button(group_frame, text="+ Añadir Grupo", command=self.add_group_selector)
        add_group_button.grid(row=1, column=0, columnspan=2, pady=5)

        # --- Selección de Columna ---
        col_frame = ttk.LabelFrame(main_frame, text="Variable a Analizar")
        ...
        self.col_combo = ttk.Combobox(col_frame, textvariable=self.column_var, values=[], state="readonly", width=50)
        self.col_combo.grid(row=0, column=1, sticky="ew", padx=5, pady=5)

        # --- Opciones Estadísticas ---
        stats_frame = ttk.LabelFrame(main_frame, text="Supuestos Estadísticos")
        stats_frame.grid(row=4, column=0, columnspan=3, sticky="ew", pady=10)

        ttk.Checkbutton(stats_frame, text="Asumir Datos Paramétricos (distribución normal)", variable=self.parametric_var).pack(side=tk.LEFT, padx=10, pady=5)
        ttk.Checkbutton(stats_frame, text="Datos Pareados (misma unidad de muestreo en todos los grupos)", variable=self.paired_var).pack(side=tk.LEFT, padx=10, pady=5)

        # --- Botones de Acción ---
        action_button_frame = ttk.Frame(main_frame)
        action_button_frame.grid(row=5, column=0, columnspan=3, sticky="e", pady=(20, 0))

        ttk.Button(action_button_frame, text="Generar Gráfico y Guardar", command=self.generate_analysis).pack(side=tk.LEFT, padx=5)
        ttk.Button(action_button_frame, text="Cancelar", command=self.destroy).pack(side=tk.LEFT, padx=5)
    ...
    def update_available_groups(self, event=None):
        """Actualiza la lista de grupos disponibles basada en la frecuencia seleccionada."""
    ...
    def add_group_selector(self, initial_value=""):
        ...
        display_names = [g[0] for g in self.available_groups]
        combo = ttk.Combobox(frame, textvariable=group_var, values=display_names, state="readonly", width=35)
        combo.grid(row=0, column=0, sticky="ew", padx=(0, 5))
        combo.bind("<<ComboboxSelected>>", self.update_available_columns) # Actualizar columnas al cambiar grupo

        # Botón para eliminar esta fila (deshabilitado para las 2 primeras)
        remove_button = ttk.Button(frame, text="-", width=3,
    ...
    def remove_group_selector(self, frame_to_remove, var_to_remove):
        ...
        if len(self.group_selector_frames) <= 2:
            messagebox.showwarning("Acción no permitida", "Se requieren al menos dos grupos.", parent=self)
            return
        ...
        try:
            ...
            # Re-indexar grid de los frames restantes (opcional, pero bueno para consistencia)
            for i, frame in enumerate(self.group_selector_frames):
                frame.grid(row=i, column=0, sticky="ew", pady=2)

            # Actualizar columnas disponibles ya que la selección de grupos cambió
            self.update_available_columns()

        except ValueError:
            logger.error("Intento de eliminar un frame de grupo que no está en la lista.")


    def get_selected_group_keys(self) -> List[str]:
        """Obtiene las claves originales de los grupos seleccionados y validados."""
        selected_keys = []
        ...
        for group_var in self.selected_group_vars:
            ...
            if display_name in selected_display_names:
                 messagebox.showerror("Error de Selección", f"El grupo '{display_name}' está seleccionado más de una vez.", parent=self)
            ...
            if original_key:
                ...
                logger.error(f"No se encontró la clave original para el grupo seleccionado: {display_name}")
                valid = False
                break

        if not valid or len(selected_keys) < 2:
            return [] # Devolver vacío si no es válido o no hay suficientes grupos
    ...
    def update_available_columns(self, event=None):
        ...
        if selected_freq and selected_calc and len(selected_group_keys) >= 2:
            try:
                self.available_columns = self.analysis_service.get_common_columns_for_groups(
                    self.study_id, selected_freq, selected_calc, selected_group_keys
                )
                self.col_combo['values'] = self.available_columns
                if self.available_columns:
                     # Seleccionar la primera columna por defecto si hay alguna
                     # self.column_var.set(self.available_columns[0])
                     pass # O dejar vacío para que el usuario elija explícitamente
                else:
                     messagebox.showinfo("Sin Columnas Comunes",
                                         "No se encontraron columnas de datos comunes para la combinación de cálculo y grupos seleccionada.",
                                         parent=self)

            except Exception as e:
                logger.error(f"Error obteniendo columnas comunes: {e}", exc_info=True)
                messagebox.showerror("Error", f"No se pudieron cargar las columnas comunes:\n{e}", parent=self)
                self.available_columns = []


    def generate_analysis(self):
        """Valida la configuración y llama al servicio para generar el análisis."""
        ...
        if not analysis_name:
            messagebox.showerror("Error de Validación", "Por favor, ingrese un nombre para el análisis.", parent=self)
            return
        # Validar caracteres inválidos en nombre (repetido de AnalysisService, bueno tenerlo en UI también)
        invalid_chars = r'<>:"/\|?*'
        ...
        if not selected_freq or not selected_calc:
            messagebox.showerror("Error de Validación", "Seleccione Frecuencia y Cálculo.", parent=self)
            return
        if len(selected_group_keys) < 2:
            messagebox.showerror("Error de Validación", "Seleccione al menos dos grupos válidos y distintos.", parent=self)
            return
        if not selected_col:
            messagebox.showerror("Error de Validación", "Seleccione la columna (variable) a analizar.", parent=self)
            return
        ...
        try:
            ...
            result = self.analysis_service.perform_individual_analysis(self.study_id, config)
            messagebox.showinfo("Análisis Generado",
            ...
            self.destroy() # Cerrar diálogo de configuración
             ...
             logger.warning(f"Error de validación o datos al generar análisis '{analysis_name}': {e}")
             messagebox.showerror("Error al Generar Análisis", f"{e}", parent=self)
        except Exception as e:
             logger.critical(f"Error inesperado al generar análisis '{analysis_name}': {e}", exc_info=True)
        ...
        def __init__(self):
             ...
             class DummySettings:
                  def get_descriptor_alias(self, desc):
                       return {'CMJ': 'Salto CM', 'PRE': 'Antes', 'POST': 'Despues'}.get(desc)
